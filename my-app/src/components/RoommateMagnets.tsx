import type { FridgeMagnet } from '../lib/types'

function MagnetSticker({ magnet }: { magnet: FridgeMagnet }) {
  const tilt = magnet.name.length % 2 === 0 ? -2 : 2
  ...
  return (
    <figure
      ...
      style={{
        backgroundColor: magnet.avatar_color,
        transform: `rotate(${tilt}deg)`,
      }}
    >
      <figcaption className="sr-only">{magnet.name}</figcaption>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 text-left">
          ...
          <p style={{ fontFamily: '"Patrick Hand", cursive' }} className="mt-1 text-base leading-snug text-[#24313A]/92">
...
export default function RoommateMagnets({ magnets }: { magnets: FridgeMagnet[] }) {
  return (
    <section className="fridge-well flex flex-col gap-6">
      <header className="px-4 pt-2">
        ...
        <p style={{ fontFamily: '"Patrick Hand", cursive' }} className="mt-1 max-w-[18rem] text-lg leading-tight text-[#24313A]/90">
      ...
      <div className="flex flex-wrap gap-4 px-2 pb-2">
        ...
        {!magnets.length && (
          <p style={{ fontFamily: '"Patrick Hand", cursive' }} className="text-base italic text-[#24313A]/76">
